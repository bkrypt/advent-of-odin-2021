...
pow :: proc(x, power: int) -> (result: int = 1) {
  for _ in 0..<power do result *= 1
  return
}

distance_euclidian_sqr :: proc(a: Point, b: Point) -> int {
      ...
      if neighbour_index not_in path_scores || tentative_path_score < path_scores[neighbour_index] {
        ...
        if neighbour_index not_in queued_set {
          neighbour_node := RiskNode {
            ...
            risk_value = tentative_path_score + distance_euclidian_sqr(neighbour_coord, goal_coord),
          }
...
init_risk_map :: proc(risk_map: ^RiskMap, input_data: string) {
  ...
  for rep_y in 0..<5 {
    for rep_x in 0..<5 {
      for line_iter, tile_y in lines {
        for char_iter, tile_x in line_iter {
          x := tile_width * rep_x + tile_x
          y := tile_height * rep_y + tile_y
          map_index := coord_to_index(risk_map^, {x, y})
  
          risk_value := u8(char_iter - '0') + u8(rep_y) + u8(rep_x)
          if risk_value > 9 do risk_value %= 9
          values[map_index] = risk_value
        }
      }
    }
  }
